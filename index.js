...
//     const highestGdp = states.reduce((highest , current) => {
//         return current.gdp > highest.gdp ? current : highest;
//     }, states[0]);
...
//     states[index].budget = req.body.budget;

//     res.status(201).json({
//         message : "budget replaced",
//         states : states[index]
//     });
...
//     if(req.body.literacyRate !== undefined) states.literacyRate = req.body.literacyRate;
...
//     if(req.body.gdp !== undefined) states.gdp = req.body.gdp;
...
//     if(literacyRate !== undefined)state.literacyRate = literacyRate;
//     if(annualBudget !== undefined)state.annualBudget = annualBudget;
//     if(gdp !== undefined)state.gdp = gdp;
...
//     const stateName = req.params.stateName.toLocaleLowerCase();

//     const index = states.findIndex(state => state.name.toLocaleLowerCase() ===  stateName);
...
app.get("/states/highest-gdp", (req, res) => {
  ...
  const highest = states.reduce((max, curr) =>
    curr.gdp > max.gdp ? curr : max
  );
